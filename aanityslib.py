...
def aanita(savepath, t):
    ...
    wf.close()
    """"
    x, f_s = sf.read(completeName)
    kohinoitu = kh.lisaa_kohina(x, 0.1)
    sd.play(kohinoitu, fs)
    time.sleep(3)
    reduced_noise = nr.reduce_noise(kohinoitu, f_s)
    sd.play(reduced_noise, fs)
    time.sleep(3)
    trim = l.leikkaa(reduced_noise, 1)
    sf.write(completeName, trim, f_s)
    """
